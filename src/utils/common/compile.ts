        ...
        throw new SyntaxError(
            result
                .diagnostics!.map((item) => {
                    const upThroughError = content.slice(0, item.start);
                    const afterError = content.slice(item.start! + 1);
                    const lineNumber = upThroughError.split("\n").length;
                    const startIndexOfThisLine =
                        upThroughError.lastIndexOf("\n");
                    ...
                    return (
                        `Line #${lineNumber}: ${item.messageText}` +
                        "\n " +
                        lineText
                    );
                })
